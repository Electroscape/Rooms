...
void setup() {
  wdt_disable();
  Serial.begin(115200);
  Serial.println(); Serial.println("===================date 01.04.2020====================="); Serial.println();
...
bool decontamination() {
  ...
  for(int z=0; z<35; z++){
    ...
    if (z==3) {relay.digitalWrite(REL_FOG_PIN, !REL_FOG_INIT); Serial.println("Fog: on");
    }
    else if (z==13) {relay.digitalWrite(REL_FOG_PIN, REL_FOG_INIT); Serial.println("Fog: off");
    }
    else if (z==15) {relay.digitalWrite(REL_FAN_OUT_BIG_PIN, !REL_FAN_OUT_BIG_INIT); Serial.println("Fan OUT big: on");}
