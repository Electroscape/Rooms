 ...
 *
 *		v1.1
 *		- Last Changes 04.12.2018
 *		- Remove the delay() function for the 3 mins reseting check at Lab's door
 *		-
 */
/*==========================================================================================================*/
  ...
  if(!iTrigger.digitalRead(REED_0_PIN) && !firstRun && !entranceOpen) {
    ...
    Serial.println("Lab door: open (2nd check)");
      if(!iTrigger.digitalRead(REED_1_PIN)) {
        ...
        if( decontamination(true) ) { Serial.println("Decontamination: end"); }
        firstRun = true;
      }
  }
...
bool decontamination( bool cleanroomLocked ) {
  ...
  for(int z=0; z<35; z++){
    if (z<5) {relay.digitalWrite(REL_FOG_PIN, !REL_FOG_INIT); Serial.println("Fog: on");}
    else {relay.digitalWrite(REL_FOG_PIN, REL_FOG_INIT); Serial.println("Fog: off");}
    if (z>5) {relay.digitalWrite(REL_FAN_OUT_BIG_PIN, !REL_FAN_OUT_BIG_INIT); Serial.println("Fan OUT big: on");}
