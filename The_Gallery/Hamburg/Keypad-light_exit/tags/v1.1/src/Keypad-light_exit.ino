...
void setup() {
  ...
  relay.digitalWrite(EXP_TUER_PIN, LOW);       /* Tür-Relais an, damit Ruhestromtür verschlossen bleibt. NC -> Tuer offen */
	Serial.print("Tuer Pin ["); Serial.print(EXP_TUER_PIN); Serial.println("] auf LOW gezogen");
	delay(20);
  relay.digitalWrite(EXP_LICHT_PIN, LOW);      /* Licht-Relais an, damit Licht initial aus.              NC -> Licht an   */
	Serial.print("Licht Pin ["); Serial.print(EXP_LICHT_PIN); Serial.println("] auf LOW gezogen");
  oled.println(F("ok."));
