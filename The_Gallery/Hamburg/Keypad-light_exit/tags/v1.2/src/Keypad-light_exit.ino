...
const String title = String("ZELLE SICHERUNGSKASTEN TUEROEFFNER v1.0 beta");
...
// ______________________EINSTELLUNGEN______________________
	...
	// LCD
		#define LCD_I2C_ADD					 0x27			/* vorgegeben
...
/*==LCD=====================================================================================================*/
LiquidCrystal_I2C lcd(LCD_I2C_ADD, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE);    // Nano: A4 -> SDA, A5 -> SCL
bool UpdateLCD = true;
...
String secsysString = String("ONLINE");
...
void LCD_Update() {
	if ( (( (millis() - UpdateLCDAfterDelayTimer) > UpdateLCDAfterDelay)) && !KeypadTyping) {UpdateLCD = true;} //|| (((millis() - UpdateLCDAfterStatusChangeTimer) > UpdateLCDStatusChange) && statusLCDChange) { UpdateLCD = true; }
	...
	if (UpdateLCD) {
		...
		if (KeypadTyping) {
			...
			delay(1000);
			//UpdateLCDAfterStatusChangeTimer = millis();
			//statusLCDChange = true;
			UpdateLCD = true;
			KeypadCodeWrong = false;
		}
		else { LCD_4x20_homescreen(); }
	}
}

void LCD_4x20_homescreen() {
	lcd.clear();
	lcd.setCursor(2, 0);
	lcd.print("SECURITY SYSTEM");
	lcd.setCursor(7, 1);
	lcd.print(secsysString);
	lcd.setCursor(0, 3);
	lcd.print("DANGER: HIGH VOLTAGE");
}

void LCD_4x20_keypadscreen() {		// ToDo: automatisch zentrieren
	int cursor = 6;
	...
	lcd.setCursor(6, 3);
	for ( int i=0; i<strlen((passLight.guess))+1; i++) {
		lcd.print(String(passLight.guess[i]));
	}
