 ...
 *
 *		v1.0
 *		- It's buildable, so you can test it
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println(); Serial.println("==============FuseBox 03.01.2019=============");
...
bool fuseCheck() {
  for( int i=0; i<5; i++ ) {
    ...
    if( fuseState[i] != iFuse.digitalRead( inputPinArray[i] ) ) {
      ...
      fuseChange = true;
      //Serial.print("Fuse "); Serial.print(i+1); Serial.print(": "); Serial.println(fuseState[i]);
  //    delay(1000);
      }
  }
  Serial.println();
...
void Keypad_Update() {
  MyKeypad.getKey();
	...
	if(strlen((pass_fusebox.guess))== 4 && doorClosed){
