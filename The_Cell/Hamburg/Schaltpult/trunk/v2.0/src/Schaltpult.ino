...
#define INPUT_I2C_ADD        0x39         // Switch Button
...
void setup() {
  ...
  if( relay_Init() 	)	{Serial.println("Relay:   ok");	} // give sometime lid still open after restart
...
void loop() {
  ...
  if (!doorClosed && !gameEnd){
    ...
    if (buttonState && !gameEnd){
      ...
      delay(3000);
      relay.digitalWrite( REL_3_PIN, REL_3_INIT );
      //relay.digitalWrite( REL_1_PIN, REL_1_INIT ); //no need to lock lid afterwards 
      gameEnd = true;
    }
    delay(250);
  }

  if (iButton.digitalRead(inputPinArray[0]) && !gameEnd){
    Serial.println("Door open");
    relay.digitalWrite( REL_2_PIN, !REL_2_INIT );
    relay.digitalWrite( REL_3_PIN, !REL_3_INIT );
    wdt_reset();
...
void checkPassword() {
	if ( passDoor.evaluate() ) {
		...
		Serial.println( "Control Box open" );
		passwordReset();
	}
	else {
